...
|---|------|-------------|---------|
| 1 | **Systems architecture diagram** (2f) | Diagram image or mermaid in markdown | Draw a *systems thinking* diagram (NOT the tech stack). Show: user's stuck moment → emotional loops (guilt, avoidance, overwhelm) → existing tools (blockers, trackers) failing → where Micro-Deck's initiation ritual breaks the cycle. Label 2-3 leverage points. This is for the presentation — make it visual. |
